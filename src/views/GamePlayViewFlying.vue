...
const loadAssets = async () => {
    const backgroundTexture = await Assets.load('/images/background/flying_background.jpg');
    const frames = [];
    for (let i = 1; i <= 90; i++) {
        const frameNumber = i.toString().padStart(4, '0');
        const texture = await Assets.load(`/images/flying_frames/frame_${frameNumber}.png`);
        frames.push(texture);
    }
...
const startPixiAndTM = async () => {
    ...
    const flyingSprite = new Sprite(frames[0]);
    flyingSprite.x = app.value.screen.width / 2;
    const gameOverLimit = app.value.screen.height - 100;
    const successLimit = 100;
    flyingSprite.y = app.value.screen.height - 200;
    flyingSprite.anchor.set(0.5);
    app.value.stage.addChild(flyingSprite);
    ...
    function updateRunnerAnimation() {
        if (!isGameOver.value) {
            ...
            if (frameCounter >= frameInterval) {
                ...
                flyingSprite.texture = frames[frame];
                frameCounter = 0;
            }
        }
    }
    ...
    async function updateCounter() {
        ...
        if (newCounterValue > counterValue && !isGameOver.value) {
            const deltaY = (newCounterValue - counterValue) * countMoveSpeed;
            const deltaX = 100 * Math.sin(Date.now() / 500);

            const leftLimit = 420;
            const rightLimit = app.value.screen.width - flyingSprite.width;

            let newX = flyingSprite.x + deltaX;
            newX = Math.max(leftLimit, Math.min(newX, rightLimit));

            gsap.to(flyingSprite, {
                y: flyingSprite.y - deltaY,
                x: newX,
                duration: 0.5,
                ease: 'power2.out'
            });
    ...
    function moveRunnerDown() {
        if (!isGameOver.value) {
            flyingSprite.y += penaltyMoveSpeed;
            if (flyingSprite.y >= gameOverLimit) {
                flyingSprite.y = gameOverLimit;
                gameOver();
            }
        }
    }
    ...
    function checkRunnerDown() {
        if (flyingSprite.y <= successLimit && !isGameOver.value) {
            flyingSprite.y = successLimit;
            success();
        }
    }
